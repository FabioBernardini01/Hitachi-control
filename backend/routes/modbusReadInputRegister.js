...
router.post('/readInputRegister', authenticateJWT, async (req, res) => {
  const { name, address = 0, length = 1 } = req.body;
  const db = req.db;
  const companyId = req.user.company_id;

  if (!name) {
    return res.status(400).json({ message: 'Nome stampante richiesto.' });
  }
  ...
  try {
    ...
    if (result.rows.length === 0) {
      console.warn(`[READ INPUT] Stampante "${name}" non trovata per company_id=${companyId}`);
   ...
   // console.log(`[READ INPUT] Risposta agent per comando id=${commandId}:`, resultCmd.result);
    ...
    console.error('Errore nella lettura degli Input Register:', error.code, error.message);
    res.status(500).json({
      error: true,
